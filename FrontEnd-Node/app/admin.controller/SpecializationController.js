...
const SpecRepository = require('../admin.repository/SpecializationRepository');
...
router.put('/update-specialization/:id', (req, res, next) => {
    const body = req.body;
    const param = req.params;
    SpecRepository.updateSpecialization(param.id, body).then((data) => {
        res.status(data.status).send(data.message)
    }).catch(err => {
        res.status(err.status).send(err.message)
    })
