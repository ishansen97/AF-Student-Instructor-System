...
const FacultyRepository = require('../admin.repository/FacultyRepository');
...
router.put('/update-faculty/:id', (req, res, next) => {
    const body = req.body;
    const param = req.params;
    FacultyRepository.updateFaculty(param.id, body).then((data) => {
        res.status(data.status).send(data.message)
    }).catch(err => {
        res.status(err.status).send(err.message)
    })
